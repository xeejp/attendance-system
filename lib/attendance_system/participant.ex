...
#    if data.page == "waiting", do: data = data |> put_in([:participants, id, :active], true)
  ...
  def press_numeric(data, id, num) do
    data = if data.participants[id].timestamp < data.time - data.max do
       data |> put_in([:participants, id, :number], [])
    else
      data
    end
    ...
    if(length(data.participants[id].number) == data.combo) do
     ...
     case Enum.join(data.participants[id].number, "") do
       ^a -> Actions.success(data |> put_in([:participants, id, :answered], true) |> put_in([:participants, id, :finishtime], time), id)
       ^b -> Actions.success(data |> put_in([:participants, id, :answered], true) |> put_in([:participants, id, :finishtime], time), id)
        _ -> {:ok, %{"data" =>  (data |> put_in([:participants, id, :number], 
           if length(data.participants[id].number) >= data.combo do
        ...
        {:ok, %{"data" =>  (data |> put_in([:participants, id, :number], 
           if length(data.participants[id].number) >= data.combo do
             tl(data.participants[id].number)
           else
             data.participants[id].number
